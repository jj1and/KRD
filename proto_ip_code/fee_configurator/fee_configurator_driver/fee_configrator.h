...
#define DEFAULT_CONTROL_ADDR 0x00010010
...
typedef struct FEE_Config {
    ...
    u32 ControlAddr[CHANNEL_NUM];
    u32 ThresholdAddr[CHANNEL_NUM];
    u32 AcquisitionAddr[CHANNEL_NUM];
    u32 BaselineAddr[CHANNEL_NUM];
} FEE_Config;

#define FEEConfig_BaseAddress(CfgPtr) ((CfgPtr)->BaseAddress)

#define FEEConfig_IsConfigState(CfgPtr, Channel) \
    (((FEEConfig_ReadReg((CfgPtr)->BaseAddress, CONTROL_ADDR_OFFSET + Channel * CHANNEL_OFFSET) & CONTROL_STATE_MASK) == CONFIG_MODE) ? TRUE : FALSE)
#define FEEConfig_IsStopState(CfgPtr, Channel) (((FEEConfig_ReadReg((CfgPtr)->BaseAddress, CONTROL_ADDR_OFFSET + Channel * CHANNEL_OFFSET) & CONTROL_STATE_MASK) == STOP_MODE) ? TRUE : FALSE)
#define FEEConfig_IsNomalAcquireMode(CfgPtr, Channel) \
    (((FEEConfig_ReadReg((CfgPtr)->BaseAddress, CONTROL_ADDR_OFFSET + Channel * CHANNEL_OFFSET) & ACQUIRE_MODE_MASK) == NORMAL_MODE) ? TRUE : FALSE)
#define FEEConfig_IsCombinedAcquireMode(CfgPtr, Channel) \
    (((FEEConfig_ReadReg((CfgPtr)->BaseAddress, CONTROL_ADDR_OFFSET + Channel * CHANNEL_OFFSET) & ACQUIRE_MODE_MASK) == COMBINED_MODE) ? TRUE : FALSE)
